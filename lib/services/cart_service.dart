...
class CartService {
  ...
  Future<void> addToCart(CartItem item) async {
    ...
    if (response.statusCode != 200 && response.statusCode != 201) {
      throw Exception('Failed to add product to cart with status code: ${response.statusCode}');
    }
  }
  ...
  Future<List<CartItem>> getCartItems(int userId) async {
    ...
    if (response.statusCode == 200) {
      ...
      throw Exception('Failed to load cart items with status code: ${response.statusCode}');
    }
  }
  Future<void> deleteCart(int cartId) async {
    ...
    if (response.statusCode != 200 && response.statusCode != 204) {
      throw Exception('Failed to delete cart with status code: ${response.statusCode}');
    }
  }
}